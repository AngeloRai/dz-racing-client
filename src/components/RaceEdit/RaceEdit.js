...
function RaceCreate() {
  ...
  const [raceResultPilots, setRaceResultPilots] = useState([]);
  ...
  useEffect(() => {
    const fetchPilots = async () => {
      try {
        ...
        setListOfPilots(fetchedListPilots.data); //fetches all pilot names
        const fetchedRaceResultPilots = await api.get("/race-results")
        setRaceResultPilots(fetchedRaceResultPilots.data)
        const listCircuits = await api.get("/circuits");
        setCircuitNames(listCircuits.data);
        console.log(fetchedRaceResultPilots.data);
